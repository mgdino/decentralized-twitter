...
type AuthContext = {
  ...
  signUpWithGoogle: (wallet: string) => Promise<void>;
  signInWithGoogle: (wallet: string) => Promise<void>;
};
...
export function AuthContextProvider({
  ...
  const signUpWithGoogle = async (wallet: string): Promise<void> => {
    try {
      // const provider = new GoogleAuthProvider();
      // await signInWithPopup(auth, provider)  ;
      const auth2 = getAuth();
      signInWithEmailAndPassword(auth2, wallet + "@dx.com", "password")
      .then((userCredential) => {
        // Signed up 
  ...
  const signInWithGoogle = async (wallet: string): Promise<void> => {
    try {
      // const provider = new GoogleAuthProvider();
      // await signInWithPopup(auth, provider)  ;
      const auth2 = getAuth();
      createUserWithEmailAndPassword(auth2, wallet + "@dx.com", "password")
      .then((userCredential) => {
        // Signed in 
        const user = userCredential.user;
        // ...
